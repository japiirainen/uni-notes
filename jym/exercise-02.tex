...
\end{scope}
\end{tikzpicture}
\caption{Kohtien (a)--(c) joukkojen visualisoinnit Vennin diagrammeina.
Potenssijoukon \(\powerset{B}\) visualisointi ei onnistu Vennin diagrammilla.}
\end{figure}
\end{solution}
...
\begin{solution}
Kirjoitetaan ensin implikaatio määritelmän mukaan predikaattilogiikan
kielellä. Saadaan
\[
\begin{aligned}
\forall x \in X \colon \bigl(x \in A \cup (B \cap C) &\implies x \in (A
\cup B) \cap (A \cup C)\bigr) \\
\iff \forall x \in X \colon \bigl(x \in A \lor (x \in B \land x \in C)
&\implies ((x \in A) \lor (x \in B)) \land ((x \in A) \lor (x \in C))\bigr).
\end{aligned}
\]
Todistetaan nyt väite vastatodistuksella. Olkoon \(x \in X\) mielivaltainen.
Tehdään vastaoletus: oletetaan, että
\[
x \in A \cup (B \cap C) \quad \text{\emph{ja}} \quad x \notin (A \cup
B) \cap (A \cup C).
\]
Koska \(x \in A \cup (B \cap C)\), \(x \in A\) tai \(x \in B \cap C\).

Ensimmäisesta tapauksesta seuraa \(x \in A \cup B\) sekä
\(x \in A \cup C\), joten \(x \in (A \cup B) \cap (A \cup C)\), mikä
on ristiriidassa vastaoletuksen kanssa.

Toisessa tapauksessa, \(x \in B \cap C\), joten \(x \in B\) sekä \(x \in C\).
Näistä seuraa \(x \in A \cup B\) sekä \(x \in A \cup C\), joten
\(x \in (A \cup B) \cap (A \cup C)\), mikä on ristiriidassa vastaoletuksen
kanssa.

Molemmissa tapauksissa päädytään ristiriitaan vastaoletuksen kanssa. Siispä
vastaoletuksen on oltava väärä, joten
\[
