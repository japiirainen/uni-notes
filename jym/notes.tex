...
We begin with basic mathematical notation.
...
\begin{definition}
  ...
  \(x \in A\); otherwise, we write \(x \notin A\). A set is determined entirely
  by its elements: two sets are equal exactly when they have the same elements.
  Thus the order in which the elements are listed, and any repetition among
  them, do not affect the set.
\end{definition}
