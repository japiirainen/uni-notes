...
\exerciseheading{Analyysi I}{Harjoituksen 2 ratkaisut}
...
\begin{solution}
  Olkoon \(\epsilon > 0\). Valitaan \(N \in \NN\) jolla \(N >
  \maxset{6, \frac{24}{\epsilon}}\). Kun \(n > N\), niin \(n > 6\),
  joten \(n - 3 > n/2\) ja siten
  \(\frac{12}{n - 3} < \frac{24}{n}\). Täten
  \[
    \abs{x_n - 4} = \frac{12}{n - 3} < \frac{24}{n} < \frac{24}{N}
    < \epsilon,
  \]
  eli \(\lim_{n \to \infty} x_n = 4\).

  \smallskip
  \noindent\textit{Huomautus.} Tehtävän kaava ei ole määritelty, kun
  \(n = 3\). Voimme määritellä esimerkiksi \(x_3 = 0\), sillä yhden
  jäsenen arvo ei vaikuta lukujonon raja-arvoon.
\end{solution}
...
\begin{solution}
  ...
  Valitaan \(\epsilon = 1\). Suppenemisen määritelmän nojalla on olemassa
  ...
  \abs{a} + 1}\).
  Koska \(n\) on parillinen, \(y_n = -n\), joten käänteisen
  ...
  Oletetaan, että \(\lim_{n \to \infty} y_{2n} = a\). Valitaan
  \(\epsilon = 1\). Suppenemisen määritelmän nojalla on olemassa
  \(N \in \NN\) siten, että kaikilla \(n > N\) pätee
  \[
    \abs{y_{2n} - a} < 1.
  \]
  Valitaan \(n \in \NN\) niin suuri, että \(n > N + \abs{a} + 1\).
  Koska \(y_{2n} = -2n\), käänteisen kolmioepäyhtälön nojalla
  \[
...
\begin{solution}
  ...
  \[
    \abs{x_n - m} = \frac{1}{n} < \frac{1}{N} < \epsilon.
  \]
...
\begin{solution}
  Olkoon \(\epsilon > 0\). Lukujono \((y_n)\) suppenee, joten se on rajoitettu.
  Täten on olemassa \(M > 0\), jolla \(\abs{y_n} \leq M\) kaikilla
  \(n \in \NN\). Valitaan \(N_1 \in \NN\) siten, että kun \(n > N_1\), niin
  \[
  ...
  \]
  Nimittäjässä käytetään lukua \(\abs{a} + 1\), jotta se on positiivinen
  myös silloin, kun \(a = 0\). Lisäksi \(\frac{\abs{a}}{\abs{a} + 1} < 1\).
